...
public class AvlTree {
  ...
  public void preOrder(AvlNode t) {
    ...
    public void printHeights(AvlNode t, int[] count){
        count[0]++;
        if( t.getLeft() !=null)
            this.printHeights(t.getLeft(), count);
        
         
        if( t.getRight() != null)
            this.printHeights(t.getRight(), count);
    }
    
      public int depth(String value, AvlNode t) {
    ...
    if (value.compareTo(t.getValue())<0) {
      ...
      return 1+depth(value, t.getLeft());
      
    }
    
    else  if(value.compareTo(t.getValue())>0) {
      
      return 1+depth(value, t.getRight());
